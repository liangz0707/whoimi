...
首先分析一种错误的理解：反射半球的表面积应该是$2\pi r^2$，反射光照总能量为$\Phi$，所以的单位幅通量为$$\frac{\Phi}{2\pi r^2}$$，这个时候表面反射率为R，所以lambertian反射强度应该是$$\frac{R}{2\pi}$$，那为什么是$ \frac{R}{\pi}$呢？？？

这种理解错误的原因是：**Lambertian模型并非描述的是入射光全部被平均反射，而是任何一个反射方向单位幅通量一样**，两者的区别在于：前者考虑的是**入射总量**确定后计算**反射辐通量**是多少，后者考虑的是**反射辐通量相等**的时候**入射辐通量**是多少。

因为我们计算光照考虑的是辐通量（光照强度）而不是光照总量。所以这里是$ \frac{R}{\pi}$。

这个$\pi$是怎么来的？

对**幅通量**根据Lambertian定律（反射幅通量和$cos(\theta)$成正比）进行半球积分得到表面接收的**幅通量**。
